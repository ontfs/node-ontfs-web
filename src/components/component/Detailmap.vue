...
export default {
  ...
  computed: {
    ...
    detailMapCode() {
      return this.$store.getters.detailMapCode || {}
    }
    // mapLog() {
    //   return {
  ...
  data() {
    return {
      map: null,
      mapboxurl:
        'https://api.mapbox.com/styles/v1/tony1duan/cjyqy7w2s02lh1cqoqoppsx5g/tiles/256/{z}/{x}/{y}@2x?access_token=pk.eyJ1IjoidG9ueTFkdWFuIiwiYSI6ImNqeW82NDd4czExY3czYnF1eGhxZ29hMnEifQ.hzvWApDHuwy26QZFZArSkQ',
      option: {
        zoom: 6,
        maxZoom: 18,
        minZoom: 3,
  ...
  watch: {
    detailMapCode: function() {
      this.latitude = this.detailMapCode.latitude
      this.longitude = this.detailMapCode.longitude
      this.mapInit()
    }
    // mapLog: function() {
    //   this.latitude = this.mapLog.latitude
    //   this.longitude = this.mapLog.longitude
    //   this.mapInit()
    // }
  },
  mounted() {},
