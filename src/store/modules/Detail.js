...
export default {
  ...
  actions: {
    async getNodeOnchainInfo({ commit }, params) {
      let url = expURL + '/v2/nodes/on-chain-info?public_key=' + params.pk
    ...
    async getNativeBalance({ commit }, params) {
      let url = expURL + '/v2/addresses/' + params.address + '/native/balances'
      let ret = await axios.get(url)
      commit('setnativebalance', ret.data.result)
    },
    async getOep4Balance({ commit }, params) {
      let url = expURL + '/v2/addresses/' + params.address + '/oep4/balances'
      let ret = await axios.get(url)
      commit('setoep4balance', ret.data.result)
    },
    async getOep5Balance({ commit }, params) {
      let url = expURL + '/v2/addresses/' + params.address + '/oep5/balances'
      let ret = await axios.get(url)
      commit('setoep5balance', ret.data.result)
    },
    async getOep8Balance({ commit }, params) {
      let url = expURL + '/v2/addresses/' + params.address + '/oep8/balances'
      let ret = await axios.get(url)
    ...
    async getTxList({ commit }, params) {
      ...
      if (ret.data.result.length == params.pagesize) {
        ...
        arr.total =
          params.pagesize * (params.pageno - 1) + ret.data.result.length
      }
      commit('settxlist', arr)
    },
    async getTransferList({ commit }, params) {
      let url =
        'https://node.ont.io/api/v2/addresses/' +
        params.address +
        '/transactions?direction=desc&page_size=' +
        params.pagesize +
        '&page_number=' +
        params.pageno
      let ret = await axios.get(url)
    ...
    async getVoterList({ commit }, params) {
      let url =
        'https://node.ont.io/api/v2/nodes/governance?page_size=' +
        params.pagesize +
        '&page_number=' +
        params.pageno +
        '&pk=' +
